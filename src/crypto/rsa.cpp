...
string RSA::encodeKey(BigInt modulus, BigInt exponent)
{
    ...
    writeIntToBytes<int>(m_len, buffer, ENDIAN_BIG);
    ...
    string ret = Base64::encode(buffer, 4 + m_len + e_len);
    delete[] m_bytes;
    delete[] e_bytes;
    delete[] buffer;
...
RSAKeyInfo RSA::decodeKey(string const& key)
{
    ...
    size_t size = Base64::decode(key, buffer);
    int m_len = readIntFromBytes<int>(buffer, ENDIAN_BIG);
...
void RSA::generateXteaKey(ubyte* buf, size_t len, int* xteaKey)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        *(xteaKey + i) = readIntFromBytes<int>(data + i * sizeof(int), ENDIAN_BIG);
    }
